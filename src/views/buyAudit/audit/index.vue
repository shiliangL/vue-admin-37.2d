      ...
      <table-contain  :height.sync="table.maxHeight">
        <el-table :data="table.data" slot="table" :size="table.size" :max-height="table.maxHeight" style="width: 100%;" highlight-current-row>
          ...
          <el-table-column prop="auditStaffName" label="审核人" align="center">
              <template slot-scope="scope" align="center">
                <span v-cloak v-if="scope.row.auditStatus ===2">  </span>
                <span v-cloak v-else> {{scope.row.auditStaffName}} </span>
            </template>
          </el-table-column>
...
import { fecthList } from '@/api/buyAudit/audit.js'
...
export default {
  ...
  methods: {
    ...
    click2view(index, row) {
      this.$setKeyValue(this.add, { visiable: true, data: { type: 'view', obj: row, title: '采购审核详情' }})
    },
    showAdd() {
